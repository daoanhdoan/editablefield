(function ($) {
Drupal.behaviors.editableFieldSubmit = {
  attach: function (context) {
    $('.editablefield-item').once('editablefield').each(function() {
      var $this = $(this);
      ...
      if ($singleEditableFields.length === 1 || $this.find('input[type=radio]').length > 1) {
        $this.find('.form-submit.editablefield-edit-hover').hide();
      ...
      if ($submit.length > 0) {
        $(this).bind("dblclick", function () {
          var $this = $(this);
          if ($this.find('input[type=text],input[type=checkbox],textarea,select').length == 0 || $this.find('input[type=radio]').length > 0) {
            $this.find('.editablefield-edit').trigger("click");
          }
        });
      }
    });
